...
# Function to perform the card check with timeout
def check_card(user_id, card_info):
    ...
    try:
        ...
        # Send request to Stripe API with timeout
        ...
        # Send request to final API with timeout
        ...
        if "Your card was declined." in response_text:
            return f"❌ DECLINED ❌\n\nResponse:\n{response_text[:2000]}"  # Limit response length
        else:
            return f"✅ APPROVED ✅\n\nResponse:\n{response_text[:2000]}"  # Limit response length
            
    except requests.exceptions.Timeout:
        return "⚠️ Timeout: The request took too long and was aborted."
